...
Struct(@def,@obj="",@name="",@offset=0,@TypeArray=0,@Encoding=0){
  ...
  ;Common structures
	static RECT="left,top,right,bottom",POINT="x,y",POINTS="Short x,Short y",FILETIME="dwLowDateTime,dwHighDateTime"
  			,SYSTEMTIME="Short wYear,Short wMonth,Short wDayOfWeek,Short wDay,Short wHour,Short wMinute,Short wSecond,Short Milliseconds"
  ...
  static AHKLine="UChar ActionType,UChar Argc,UShort FileIndex,LineNumber,Arg,Attribute,*AHKLine PrevLine,*AHKLine NextLine,RelatedLine,ParentLine"
  			,AHKLabel="name,*AHKLine JumpToLine,*AHKLabel PrevLabel,*AHKLabel NextLabel"
  			...
  			,AHKFunc="Name,{BIF,*AHKLine JumpToLine},*AHKFuncParam Param,Int ParamCount,Int MinParams,*AHKVar var,*AHKVar LazyVar,Int VarCount,Int VarCountMax,Int LazyVarCount,Int Instances,*AHKFunc NextFunc,Char DefaultVarType,Char IsBuiltIn"
	...
	;local variables
  local @k,@v,@enum,@size,@subdef,@split,@split1,@split2,@split3,@StructObj,@union,@unionoff,@unionoffset=0,@pointer,@suboffset,@field,@array
            ...
            If (A_LoopField){
              ...
              If Instr(A_LoopField,"{")
                ...
                If @union
                  Loop % @union
                    @array.="{"
                @split.=(@split ? "," : "") @array ((@split3 && A_Index!=1)?(@field " "):"") A_LoopField
                @array:="",@union:=""
              }
            }
          }
  ...
  Loop,Parse,@subdef,`,,%A_Space%
  {
    ...
    @StructObj["`a" @split] := @pointer ;true if field is a pointer
  ...
  If (!@obj) ;create memory for main structure only
    ...
    ,DllCall("RtlFillMemory","UPTR",@StructObj[""],"UInt",@offset,"UChar",0)
...
Struct_GetSize(@object,o=0){
  ...
  {
    If (Asc(@k)=13){ ;`r
      If @object["`a" SubStr(@k,2)]
        @v=UInt
      p:=@object["`n" SubStr(@k,2)]
      If (p_%p%="" || %@v%>p_%p%){
        If p_%p%
          o:=o-p_%p%
        p_%p%:=%@v%
...
Struct_Get(obj,@k="",opt="~"){
  If @k=
    Return obj[""]
  type:=obj["`r" @k]
	If (obj["`a" @k]){ ;Pointer
    If Asc(opt)=126
      Return Struct(type,NumGet(obj[""]+0,obj["`n" @k]+0))
    else
      Return obj[@k][opt]
	} else if (obj["`f" @k]=1 || obj["`f" @k]=3) {
    Return NumGet(obj[""]+0,obj["`n" @k]+0,type)
	} else if obj["`f" @k]=2
		Return StrGet(obj[""]+obj["`n" @k],1,obj["`v" @k])
}

Struct_Put(obj,@k="",@v=9223372036854775809,opt="~"){
  If (@v=9223372036854775809){
    Struct(obj,0,2) ;set pointer back to offset
    ,obj._SetCapacity("`n",0)
    ,obj[""]:=@k ;set new address
    ,Struct(obj,0,1) ;set offset back to pointer
    Return
  }
	type:=obj["`r" @k]
  If (obj["`a" @k]) {
    v:=Struct("AHKVar",Struct_getVar(@v))
    If (v.Contents && StrGet(v.Contents)=""){ ;its a value not a string, assume new address
      NumPut(@v,obj[""]+obj["`n" @k],(A_PtrSize=8?"UInt64":"UInt"))
    } else {
      obj._SetCapacity("`t" @k,(obj["`v" @k]="CP0" ? 1 : 2)*StrLen(@v)+2)
      StrPut(@v,obj._GetAddress("`t" @k),StrLen(@v)+1,obj["`v" @k])
      NumPut(obj._GetAddress("`t" @k),obj[""]+obj["`n" @k],(A_PtrSize=8?"UInt64":"UInt"))
    }
  } else if (obj["`f" @k]=3){  
	v:=Struct("AHKVar",Struct_getVar(@v))
    If (v.Contents && StrGet(v.Contents)=""){ ;its a value not a string, assume new address
    ...
    StrPut(@v,obj[""]+obj["`n" @k],StrLen(@v),obj["`v" @k])
  } else {
    NumPut(@v,obj[""]+0,obj["`n" @k]+0,type)
  }
  Return @v
}
...
Struct_getVar(var) {
  static getVarFuncHex:=(A_PtrSize=4)?"8B4C24088B0933C08379080375028B018B4C2404998901895104C3":"488B02837810037507488B00488901C348C70100000000C3"
         pFunc:=NumGet((pcb:=RegisterCallback("Struct_getVar"))+28+(A_PtrSize=4?0:20),0,"UInt")
   ...
   if !(pbin := DllCall("GlobalAlloc","UPTR",0,"uint",StrLen(getVarFuncHex)//2))
	...
	NumPut(1,pFunc+49+(A_PtrSize=4?0:24),0,"char")
	NumPut(pbin,pFunc+A_PtrSize)
	return pbin
}
